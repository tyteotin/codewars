...
the others. Bob observed that one number usually differs from the others in evenness. Help Bob — to check his answers, he needs
...
def iq_test(numbers):
    ...
    for i in numbers:
        if((int(i)%2) == 0):
            eCount = eCount + 1
            eLoc = numbers.index(i)
        else:
            oCount = oCount + 1
            oLoc = numbers.index(i)
    if(eCount > oCount):
        return oLoc + 1
    else:
        return eLoc + 1
