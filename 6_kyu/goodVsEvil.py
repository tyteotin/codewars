...
def goodVsEvil(good, evil):
    good_rank = [1, 2, 3, 3, 4, 10]
    evil_rank = [1, 2, 2, 2, 3, 5, 10]
    gCount = map(lambda x,y:int(x)*y, good.split(), good_rank)
    eCount = map(lambda x,y:int(x)*y, evil.split(), evil_rank)
    if(sum(gCount) > sum(eCount)):
